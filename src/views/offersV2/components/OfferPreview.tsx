...
export default function OfferPreview({ form }: Props) {
    ...
    const calc = effectivePrice({ price: base, discountType: form.discountType, discountValue: Number(form.discountValue || 0) })
    return (
        <div className="space-y-4">
            <Card className="p-4">
                ...
                <div className="mt-2 text-sm text-gray-600">{buildScheduleSummary({ startDate: form.startDate, endDate: form.endDate })}</div>
                <div className="mt-2 text-xs text-gray-500">Targets: {form.appliesTo}{form.appliesTo !== 'all' ? ` (${form.targetIds.length})` : ''}</div>
            </Card>
            <Card className="p-4">
